...
\author{Mohammad (PhD Research, NJIT)}
\date{March 2026}
...
Restricted eigenvectors improve MLP performance at $k{=}1{-}2$ on \emph{all} 9/9
datasets (Part A range: $+0.8$pp to $+31.9$pp).
...
(3)~\textit{Weak crossover} (ogbn-arxiv, WikiCS—2/9): Part~A reaches zero or slightly
negative at high $k$.
...
\caption{Dataset statistics after LCC extraction ($k{=}10$). $d_{\mathrm{eff}}$ = rank
of $\hat{A}^k X$ after QR decomposition (\texttt{src/graph\_utils.py:compute\_restricted\_eigenvectors()}).
Homophily $h$ = fraction of edges connecting same-class nodes (computed on all nodes).
\textit{\scriptsize Data: \texttt{exp2\_spectral\_analysis\_k10.json} per dataset; assembled by \texttt{generate\_paper\_artifacts\_v2.py}.}}
...
\end{itemize}
\textbf{Practical conclusion:} within our sweep $k \in \{1,\ldots,30\}$,
$d_{\mathrm{eff}}$ is approximately \emph{constant} per dataset.
The value reported in Table~\ref{tab:dataset_statistics} (measured at $k{=}10$)
is representative for all $k$.  The large reductions (CiteSeer, Coauthor-Physics)
reflect the intrinsic rank of the raw feature matrix, not a diffusion-induced collapse.
...
%   Y-axis: Part A from results.json at k=10 (sgc_mlp - restricted_std mean test acc)
...
\subsection{Fisher Score vs.\ Part A Scatter Plot}

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/figure_3_5_fisher_vs_partb_scatter.pdf}
\caption{Fisher score of $U$ (Eq.~\ref{eq:fisher_score}) vs.\ Part~A at $k{=}10$
(fixed splits). Each point is one dataset.
X-axis: \texttt{fisher\_diagnostic.json[`fisher\_score']} at $k{=}10$.
Y-axis: \texttt{results.json} mean test accuracy difference SGC$-$R+Std.
Monotonic datasets cluster in high-Fisher/high-Part-A quadrant.
Crossover datasets have near-zero Fisher and negative Part~A.
...
\midrule
\multicolumn{5}{l}{\textit{Monotonic / near-crossover datasets:}} \\
...
\noindent\textbf{Notable anomaly:} WikiCS also shows 99.3\% Fisher collapse yet remains in
the near-crossover (not crossover) regime.  This is consistent with WikiCS being a
hyperlink network where graph connectivity is dense but node categories have some
structural isolation --- Part~A $= +0.4$pp at $k{=}30$ suggests the collapse is
approaching but has not fully crossed over under fixed splits.
...
\begin{description}
  ...
  \item[\textit{Hyperlink networks}:] WikiCS.
    Pages link for many reasons beyond category.  Dense ($\bar{d}{=}38$) but retains
    some structural isolation $\to$ near-crossover (crosses under random splits only).
\end{description}
...
(3) near-crossover (WikiCS, ogbn-arxiv---reach zero at high $k$).
...
\]
Inverse eigenvalue scaling \emph{reverses} the smoothing effect of the Laplacian
projection, amplifying low-frequency (class-discriminative) components and suppressing
high-frequency (within-class noise) components.  This effectively re-creates something
closer to the original diffused feature geometry, restoring the optimization-favorable
representation.
...
\begin{enumerate}
  \item \textbf{Universal low-$k$ benefit}: Restricted eigenvectors improve MLP
    performance at $k{=}1$ on \emph{all} 9/9 datasets.  Static degradation hypotheses are
    falsified; the relationship is $k$-dependent.
