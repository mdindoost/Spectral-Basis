...
where each frequency component is weighted by $\lambda_j^k$.  The resulting
Euclidean column-variance disparity is quantitatively predicted by the eigenvalue
range of the generalized Laplacian problem: CiteSeer ($\lambda_{\max}/\lambda_{\min}
= 234\times$) incurs a $2{\times}10^9$-fold column-variance disparity, rendering
high-frequency columns near-constant; PubMed ($8.9\times$) incurs only 171-fold
disparity, explaining its smaller Part~A.  Fisher score collapse on dense graphs
...
D-orthonormalization enforces $U^\top D U = I$, which equalizes graph-weighted variance
across columns.  However, Euclidean column variance scales as $1/\lambda_j^2$ (inversely
with the squared eigenvalue), so a large eigenvalue range produces a catastrophic
column-variance disparity: high-frequency eigenvectors become near-constant, and fitting
classifier weights to near-constant columns injects noise.
...
All empirical findings in this report follow from a single structural fact:
\textbf{D-orthonormalization destroys eigenvalue-weighted variance structure}.

The Rayleigh-Ritz procedure enforces $U^\top D U = I$.  This equalization means that
in Euclidean space, the variance of column $j$ of $U$ scales as $1/\lambda_j^2$.
In $X_{\mathrm{diff}} = \hat{A}^k X$, by contrast, each frequency component is
naturally weighted by $\lambda_j^k$, which concentrates energy in the directions most
relevant to the graph's community structure.  Rayleigh-Ritz removes this weighting.
...
\begin{enumerate}
  \item \textbf{Representation hardness}: A logistic regression on $U$ is
    ...
    shortfall (MLP vs LR: at most 10pp).  The gap is inherent to the representation,
    not recoverable by nonlinearity.
  ...
  \item \textbf{The crossover}: On dense graphs, rapid diffusion uniformly spreads
    class signal across all frequency components.  SGC+MLP degrades through
    over-smoothing while $U$-space classifiers remain stable (signal is dispersed
    but present).  The crossover in Part~A is the point at which SGC's over-smoothing
    loss exceeds the D-orthonormalization penalty of $U$.
\end{enumerate}
...
causes Effect~1 to vanish within 2--4 steps (Fisher score of $\|U_i\|$ drops 95--99\%).
...
(large $\lambda_j$, within-class variation).  It partially re-introduces the
eigenvalue weighting that D-orthonormalization removed.
...
\begin{enumerate}
  ...
  \item \textbf{Eigenvalue range predicts representation difficulty}: D-orthonormalization
    produces Euclidean column-variance disparity proportional to the squared eigenvalue
    range.  CiteSeer's $234\times$ eigenvalue range produces a $2{\times}10^9$-fold
    disparity; PubMed's $8.9\times$ range produces only $171\times$.  This predicts
    the ordering of LR accuracy gaps from SGC and the shape of the $p$-vs-accuracy
    curves in Table~\ref{tab:linear_probe}.
