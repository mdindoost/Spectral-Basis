"""
SpectralGeometryMisalignment/label_rand_experiment.py

EXPERIMENT 1 — PARTIAL LABEL INFORMATION
  GCN(X_label_rand) vs GCN(U_label_rand) vs GCN_rowNorm(U_label_rand)

SETUP:
  X_label_rand : n × num_classes feature matrix.
                 Train nodes → one-hot class label.
                 Val/test nodes → random N(0,1) with xrand_seed.
  U_label_rand : Rayleigh-Ritz eigenvectors of (L, D) restricted to
                 span(X_label_rand). At most num_classes eigenvectors.
                 D-orthonormal: U^T D U = I.

THEORETICAL GUARANTEE:
  span(X_label_rand) = span(U_label_rand) by Rayleigh-Ritz construction.
  Any gap between GCN(X) and GCN(U) is optimization geometry, not
  information content — the same guarantee as the softmax experiment.

SCIENTIFIC QUESTION:
  Does pre-spreading label information globally via spectral projection
  (U_label_rand) help or hurt compared to GCN's local hop-by-hop spreading
  (X_label_rand)?

3 MODELS:
  GCN_X        : 2-layer Kipf GCN on X_label_rand (no preprocessing)
  GCN_U        : 2-layer Kipf GCN on U_label_rand (StandardScaler, fit on
                 train nodes only, applied to all)
  GCN_rowNorm_U: 2-layer Kipf GCN on U_label_rand with row-norm at each
                 layer's INPUT (Option B — same as GCN_rowNorm_Y in gcn_rand)

DIMENSION:
  X_label_rand and U_label_rand have exactly num_classes columns.
  No padding. This is intentional — confirmed by researcher.
...
XRAND SEEDS:
  3 draws (xrand_seed in {0,1,2}).
  Controls random features for val/test nodes.
  Train nodes always receive their true one-hot label regardless of xrand_seed.
  NOTE: X_label_rand is split-dependent — different splits assign different
  nodes as train → different nodes receive one-hot labels. U_label_rand is
  therefore recomputed per (xrand_seed, split) combination.

SPLITS:
  Fixed : 1 split (standard dataset split, often ~20 nodes/class)
  Random: 5 stratified 60/20/20 splits (split_seeds 0–4, from save_data.py)

OUTPUT:
  results/label_rand/{dataset}/
...
Usage:
  /home/md724/Spectral-Basis/venv/bin/python label_rand_experiment.py
  /home/md724/Spectral-Basis/venv/bin/python label_rand_experiment.py --sanity
  /home/md724/Spectral-Basis/venv/bin/python label_rand_experiment.py --dataset cora
  /home/md724/Spectral-Basis/venv/bin/python label_rand_experiment.py \\
...
class GCN(nn.Module):
    ...
    Standard 2-layer Kipf GCN.
      ...
      H^(2) = Â H^(1) W2
    Used for GCN_X (X_label_rand) and GCN_U (StandardScaler-scaled U_label_rand).
    """
...
class GCN_RowNorm(nn.Module):
    ...
    2-layer Kipf GCN with row normalization at each layer's INPUT (Option B).
      Layer 1: Â @ row_norm(U_label_rand) → fc1 → ReLU → dropout
...
def one_hot_encode(labels_subset: np.ndarray, num_classes: int) -> np.ndarray:
    """One-hot encode a subset of labels. Returns float32 (n_subset, num_classes)."""
...
def load_cached_graph(dataset_folder: str):
    ...
    These are built and cached by gcn_rand_experiment.py (or build_and_cache_graph).
    Requires gcn_rand_experiment.py to have been run first for each dataset.
    """
...
# ── X_label_rand and U_label_rand construction ────────────────────────────────

def build_x_label_rand(n: int, num_classes: int, labels: np.ndarray,
                       tr_mask: np.ndarray, xrand_seed: int) -> np.ndarray:
    """
    Build X_label_rand (n × num_classes).
      Train nodes    → one-hot encoding of their true label.
      Val/test nodes → random N(0,1) drawn with xrand_seed.

    NOTE: X_rand_base is generated for ALL n nodes with xrand_seed first.
    Train node rows are then overridden with one-hot labels.
    This ensures the random part is consistent across splits for the same xrand_seed
    (the same val/test node always gets the same random features for a given seed,
    regardless of which split seed is used).
    """
    rng     = np.random.default_rng(xrand_seed)
    X_lr    = rng.standard_normal((n, num_classes)).astype(np.float32)
    X_lr[tr_mask] = one_hot_encode(labels[tr_mask], num_classes)
    return X_lr


def compute_u_label_rand(X_lr: np.ndarray, L, D,
                         xrand_seed: int, split_label: str) -> tuple:
    """
    Compute U_label_rand via Rayleigh-Ritz on X_label_rand.

    CHECK A: X_lr train rows are one-hot (max=1, row sum=1 on train nodes).
    CHECK B: D-orthonormality of U_label_rand: U^T D U ≈ I (error < 1e-6).
    CHECK C: Shape is (n, d_eff) where d_eff ≤ num_classes.
    ...
    if ortho_err >= 1e-6:
        raise RuntimeError(
            f'[CHECK B] U_label_rand D-ortho FAILED '
            f'(xseed={xrand_seed}, {split_label}): {ortho_err:.2e} ≥ 1e-6'
        )
    print(f'    [CHECK B] U_label_rand D-ortho: {ortho_err:.2e}  [PASS]  '
...
def train_gcn_rownorm(U_t, adj_t, labels_t, tr_t, va_t, te_t,
    ...
    """GCN with row-norm at each layer input, on raw U_label_rand."""
    torch.manual_seed(seed); np.random.seed(seed)
...
def result_path(dataset, split_type, split_seed,
...
def run_dataset(dataset_folder: str, optimizers_to_run: list,
    ...
    print(f'\n{"="*70}')
    print(f'Dataset: {dataset_folder}  [label_rand_experiment]')
    print(f'{"="*70}')
    ...
    print(f'n={n:,}  num_classes={num_classes}  '
          f'X_label_rand_dim={num_classes}  '
          f'random_baseline={100.0/num_classes:.2f}%')

    # Load cached graph matrices (built by gcn_rand_experiment.py)
    ...
    for xrand_seed in XRAND_SEEDS:
        ...
        for split_type in ['fixed', 'random']:
            ...
            for split_seed in split_seeds:
                ...
                # ── Build X_label_rand for this (xrand_seed, split) ──────────
                # X_label_rand is split-dependent: train nodes get 1-hot labels,
                # val/test nodes get random features. Different splits → different
                # nodes in train → different X_label_rand.
                X_lr   = build_x_label_rand(n, num_classes, labels,
                                            tr_mask, xrand_seed)
                X_lr_t = torch.tensor(X_lr, dtype=torch.float32, device=device)

                # ── Compute U_label_rand ─────────────────────────────────────
                U_lr, d_eff, ortho_err = compute_u_label_rand(
                    X_lr, L, D, xrand_seed, split_label
                )

                # ── StandardScaler for GCN_U (fit on train nodes only) ───────
                sc = StandardScaler()
                sc.fit(U_lr[tr_mask])
                U_lr_sc   = sc.transform(U_lr).astype(np.float32)
                U_lr_sc_t = torch.tensor(U_lr_sc, dtype=torch.float32, device=device)

                # ── Raw U for GCN_rowNorm_U (row-norm inside forward pass) ───
...
def sanity_check(device):
    ...
    """
    print('\n' + '=' * 65)
    print('SANITY CHECK — label_rand_experiment, 3 models')
    ...
    X_lr   = build_x_label_rand(n, num_classes, labels, tr, xrand_seed=0)
    X_lr_t = torch.tensor(X_lr, dtype=torch.float32, device=device)

    print(f'\n  X_label_rand: shape={X_lr.shape}')
    print(f'  Train rows (first 3): max per row = {X_lr[tr][:3].max(axis=1)}  '
          f'sum per row = {X_lr[tr][:3].sum(axis=1)}  [should be 1-hot]')

    U_lr, d_eff, ortho_err = compute_u_label_rand(X_lr, L, D, 0, 'fixed_ss0')
    U_lr_t = torch.tensor(U_lr, dtype=torch.float32, device=device)
    ...
    print(f'  {"-"*44}')
    for name, fi in [('GCN_X (X_label_rand)', fi_gcnx),
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Experiment 1: GCN(X_label_rand) vs GCN(U_label_rand)'
    )
    ...
    print(f'label_rand_experiment.py  |  Device: {device}')
    ...
    n_splits   = 1 + len(RANDOM_SPLIT_SEEDS)   # 1 fixed + 5 random
    ...
    if all_records:
        ...
        # Overview table
        print('\n' + '=' * 75)
        print('OVERVIEW — label_rand_experiment')
        print('Mean across all seeds/splits/opt/lr')
