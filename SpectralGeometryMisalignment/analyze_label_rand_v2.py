"""
SpectralGeometryMisalignment/analyze_label_rand_v2.py

Post-run analysis for label_rand_v2_experiment.py.
Produces: results/label_rand_v2/analysis/analysis_report.txt

EXPERIMENT: Partial label information — v2 (revised per Yiannis, April 2026)
  X_label_rand_v2: train → one-hot label, val/test → random PROBABILITY VECTOR
                   (uniform simplex: u ~ Uniform(0,1)^C, rows normalized to sum=1)
  U_label_rand_v2: Rayleigh-Ritz of (L,D) on X_label_rand_v2, num_classes eigenvectors

  KEY DIFFERENCE FROM v1: val/test features are probability vectors (sum=1, ≥0)
  instead of N(0,1) Gaussian noise. Consistent with the one-hot label space.

3 models: GCN_X | GCN_U (StandardScaler) | GCN_rowNorm_U (row-norm, Option B)
...
Reports:
  ...
  R5    Optimizer sensitivity
  R6    Fixed vs random split
  R7    xrand draw stability
  R8    Training curves
...
def dataset_metadata(records, line):
    ...
    for ds in ALL_DATASETS:
        ...
        ft   = r.get('rand_feature_type', 'unknown')
        line(f'{ds:<22} {n:>8,} {str(nc):>8} {str(idim):>10} {str(deff):>6} '
             f'{base:>6.2f} {str(ntr):>13} {len(recs):>8}')
    line('')
    line('input_dim = num_classes (X_label_rand_v2 has num_classes columns)')
    line('d_eff     = number of Rayleigh-Ritz eigenvectors (≤ num_classes)')
    line('rand_feature_type = simplex (v2: probability vectors, sum=1)')
...
def report1(records, line):
    ...
    line('v2: val/test features = probability simplex (not Gaussian)')
    line('=' * 80)
...
def report3(records, line):
    ...
    line('Positive Gap_U = spectral projection helps GCN.')
    line('Negative Gap_U = direct features better for GCN.')
    line(f'Canonical: {CANON_OPT}, lr={CANON_LR}, {CANON_SPLIT_TYPE} split_seed={CANON_SPLIT_SEED}')
    line('NOTE: span(X_label_rand_v2) = span(U_label_rand_v2) → gap is geometry, not info.')
    line('=' * 80)
    ...
    for ds in ALL_DATASETS:
        ...
        mu  = ' *** (U helps)'  if gap_u  > 5 else (' *** (X better)' if gap_u  < -5 else '')
        mrn = ' *** (rn helps)' if gap_rn > 5 else (' *** (rn hurts)' if gap_rn < -5 else '')
        line(f'{ds:<22} {base:>5.1f} | {f(gcnx)} {f(gcnu)} {gap_u:>+7.2f}{mu} | '
             f'{f(gcnrn)} {gap_rn:>+8.2f}{mrn}')
    line('')
    line('Gap_U  = GCN_U − GCN_X        (effect of spectral projection on GCN)')
    line('Gap_RN = GCN_rowNorm_U − GCN_U (additional effect of row-norm)')

    # Pooled over all random splits
    line('')
    ...
    for mk in MODEL_KEYS:
        ...
        for ds in ALL_DATASETS:
            ...
            for opt in OPTIMIZERS:
                blr, _ = best_lr_for(recs, mk, opt)
                if blr is None:
                    cols[opt] = ('   N/A     ', float('nan')); continue
                r  = filt(recs, split_type=CANON_SPLIT_TYPE, optimizer=opt, lr=blr)
                ta = test_accs(r, mk)
...
def report5(records, line):
    ...
    line('NOTE: X_label_rand_v2 is split-dependent — different nodes get one-hot per split.')
    line('      A large Δ means the label-seeding pattern matters.')
    line('=' * 80)
    for mk in MODEL_KEYS:
        line(f'\n  Model: {mk}')
...
def report6(records, line):
    ...
    line(f'Canonical: {CANON_OPT}, lr={CANON_LR}, {CANON_SPLIT_TYPE} split_seed={CANON_SPLIT_SEED}')
    line('Per-draw mean over train_seeds={0..4}.')
    ...
    for mk in MODEL_KEYS:
        ...
        for ds in ALL_DATASETS:
            ...
            for xs in XRAND_SEEDS:
                ...
                ta = test_accs(r, mk)
                if ta:
                    draw_means.append(mf(ta)); draw_stds.append(sf(ta))
                    draw_strs.append(f'{mf(ta):6.2f}')
                else:
                    draw_strs.append('   N/A')
            if len(draw_means) < 2:
                line(f'  {ds:<22}  [insufficient data]'); continue
...
def report7(records, line):
    ...
    line(f'Canonical: {CANON_OPT}, lr={CANON_LR}, {CANON_SPLIT_TYPE} split_seed={CANON_SPLIT_SEED}')
    line('Values = mean test accuracy (%) across xrand × train seeds.')
...
def report_findings(records, line):
    ...
    line(f'Canonical = {CANON_OPT}, lr={CANON_LR}, {CANON_SPLIT_TYPE} split_seed={CANON_SPLIT_SEED}.')
    line('v2: val/test features = probability simplex (rows sum to 1, ≥ 0).')
    line('=' * 80)
    ...
    line('')
    line('─' * 78)
    line('FINDING 1: Does spectral projection (U_label_rand_v2) help or hurt GCN?')
    line('─' * 78)
    ...
    for ds in ALL_DATASETS:
        ...
        gap  = gcnu - gcnx
        gap_u_vals[ds] = gap
        m = ' *** (U helps)' if gap > 2 else (' *** (X better)' if gap < -2 else ' (near-zero)')
        line(f'  {ds:<22}: GCN_X={f2(gcnx)}%  GCN_U={f2(gcnu)}%  Gap_U={gap:>+.2f} pp{m}')
    valid = [v for v in gap_u_vals.values() if not math.isnan(v)]
    pos   = [ds for ds,v in gap_u_vals.items() if v >  2]
    neg   = [ds for ds,v in gap_u_vals.items() if v < -2]
    if valid: line(f'  Range: {min(valid):+.2f} to {max(valid):+.2f} pp')
    ...
    line('')
    line('─' * 78)
    line('FINDING 2: Row-norm effect (Gap_RN = GCN_rowNorm_U − GCN_U)')
    line('─' * 78)
    line('')
    gap_rn_vals = {}
    for ds in ALL_DATASETS:
        ...
        gap   = gcnrn - gcnu
        gap_rn_vals[ds] = gap
        m = ' *** (helps)' if gap > 2 else (' *** (hurts)' if gap < -2 else '')
        line(f'  {ds:<22}: GCN_U={f2(gcnu)}%  GCN_rowNorm_U={f2(gcnrn)}%  Gap_RN={gap:>+.2f} pp{m}')
    valid_rn = [v for v in gap_rn_vals.values() if not math.isnan(v)]
    if valid_rn: line(f'  Range: {min(valid_rn):+.2f} to {max(valid_rn):+.2f} pp')

    # Finding 3: Fixed vs random gap
    line('')
    line('─' * 78)
    line('FINDING 3: Fixed vs random split — effect on Gap_U')
    line('─' * 78)
    line('')
    line(f'  {"Dataset":<22} {"Gap_U fixed":>12} {"Gap_U random":>13} {"Δ":>8}')
    line('  ' + '-' * 58)
    for ds in ALL_DATASETS:
        fix_r  = filt(records.get(ds,[]), split_type='fixed', split_seed=0,
                      optimizer=CANON_OPT, lr=CANON_LR)
        fix_gap = mf(test_accs(fix_r,'GCN_U')) - mf(test_accs(fix_r,'GCN_X'))
    ...
    line('')
    line('─' * 78)
    line('SUMMARY — KEY NUMBERS  [VERIFIED from canonical config, v2 simplex features]')
    line('─' * 78)
    vg = [v for v in gap_u_vals.values()  if not math.isnan(v)]
    vr = [v for v in gap_rn_vals.values() if not math.isnan(v)]
    if vg: line(f'  Gap_U  range: {min(vg):+.2f} to {max(vg):+.2f} pp')
    if vr: line(f'  Gap_RN range: {min(vr):+.2f} to {max(vr):+.2f} pp')
...
def main():
    ...
    line('=' * 80)
    line('EXPERIMENT 1-v2: PARTIAL LABEL INFORMATION — FULL ANALYSIS REPORT')
    line('SpectralGeometryMisalignment / label_rand_v2_experiment.py')
    line(f'Date: {date.today().strftime("%B %Y")}')
    line('=' * 80)
    line('')
    line('EXPERIMENTAL SETUP')
    line('------------------')
    line('X_label_rand_v2: train → one-hot label. Val/test → probability simplex vector.')
    line('  (v2 change from v1: N(0,1) → uniform simplex. Rows sum to 1, all ≥ 0.)')
    line('U_label_rand_v2: Rayleigh-Ritz of (L,D) on X_label_rand_v2.')
    line('span(X) = span(U) — gap is optimization geometry, not information.')
    line('')
