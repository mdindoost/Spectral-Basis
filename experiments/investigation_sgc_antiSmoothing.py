...
for k_diff in K_DIFFUSION_VALUES:
    ...
    print('\nOur Method:')
    print(f'  Full + RowNorm MLP:    {final_results["full_rownorm_mlp"]["test_acc_mean"]*100:.2f}%')
...
methods_to_plot = ['sgc_mlp_baseline', 'full_rownorm_mlp', 'jl_binary_2nc', 'jl_ortho_2nc']
x_params = []
y_accs = []
method_names = []
...
print(f'  Best Full + RowNorm MLP: {best_full_acc:.2f}% at k={best_k}')
print(f'  SGC Baseline at k={best_k}: {best_sgc_acc:.2f}%')
print(f'  Improvement: {best_full_acc - best_sgc_acc:+.2f}pp')
